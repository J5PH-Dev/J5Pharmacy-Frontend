...
const MedicineShortage = () => {
    ...
    const [editMedicine, setEditMedicine] = useState<any>(null);  // To store the medicine being edited
    ...
    const handleEditItem = (medicineName: string) => {
        const selectedMedicine = filteredRows.find(row => row.medicineName === medicineName);
        if (selectedMedicine) {
            setEditMedicine(selectedMedicine);
            setNewStockQty(selectedMedicine.stockQty.toString()); // Pre-fill stockQty
            setOpenModal(true);
        }
    };
    ...
    // Handle Save Changes to stock quantity
    const handleSaveStockQty = () => {
        if (editMedicine) {
            // Update the stock quantity of the medicine
            const updatedRows = filteredRows.map((row) =>
                row.medicineID === editMedicine.medicineID ? { ...row, stockQty: Number(newStockQty) } : row
            );
            setFilteredRows(updatedRows);
            setSortedRows(updatedRows);  // Update sorted rows as well
    ...
    return (
        <Box sx={{ p: 3, ml: { xs: 1, md: 38 }, mt: 1, mr: 3 }}>
            ...
            {/* Modal for editing stock quantity */}
            <Modal
                open={openModal}
                onClose={handleCloseModal}
                aria-labelledby="edit-stock-quantity-modal"
            >
                <Box sx={{
                    position: 'absolute',
                    top: '50%',
                    ...
                    <Typography variant="h6" sx={{ mb: 2 }}>Edit Stock Quantity</Typography>
                    <TextField
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Button onClick={handleSaveStockQty} color="primary">Save</Button>
                    </Box>
                </Box>
            </Modal>
