...
const InventoryPage = () => {
  ...
  const HandleViewMedicine = (item: any) => {
    setSelectedItem(item);
    navigate(`/manager/inventory/view-${item.subtitle.toLowerCase().replace(/\s+/g, '-')}`); // Navigate with dynamic URL
  };

  const HandleViewMedicineGroup = (item: any) => {
    setSelectedItem(item);
    
    // Check if the clicked item is "Medicines Group"
    if (item.subtitle === 'Medicines Group') {
      navigate(`/manager/inventory/view-medicines-group`); // Navigate to the medicines group route
    } else {
      // Otherwise, navigate to the dynamic route based on the subtitle
      navigate(`/manager/inventory/view-${item.subtitle.toLowerCase().replace(/\s+/g, '-')}`);
    }
  };
